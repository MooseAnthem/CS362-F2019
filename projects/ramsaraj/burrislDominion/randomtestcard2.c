...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < iterations; i++) {
        ...
        doBaron(currentPlayer, WANTS_GOLD, &G);
        ...
        if(WANTS_GOLD == TRUE && HAS_ESTATE == TRUE){
            ...
            if(randomAssert("\tGold not increased by 4.", G.coins == (P.coins + 4))) {failed = TRUE;}
            if(randomAssert("\tG.discardCount[currentPlayer] incorrectly updated.", G.discardCount[G.whoseTurn] == P.discardCount[G.whoseTurn])) {failed = TRUE;}
            if(randomAssert("\tTop of current player's discard array not an Estate after discarding Estate.", G.discard[G.whoseTurn][P.discardCount[G.whoseTurn]] == estate)) {failed = TRUE;}
            if(randomAssert("\tG.supplyCount[] array incorrectly modified.", memcmp(&(G.supplyCount), &(P.supplyCount), sizeof(int) * (treasure_map + 1)))) {failed = TRUE;}
        }
        ...
        if((WANTS_GOLD == FALSE && ESTATE_IN_SUPPLY == FALSE) || (WANTS_GOLD == TRUE && HAS_ESTATE == FALSE && ESTATE_IN_SUPPLY == FALSE)) {
            ...
            if(randomAssert("\tCoins incorrectly modified.", G.coins == P.coins)) {failed = TRUE;}
            if(randomAssert("\tCurrent player's discard count incorrectly modified.", G.discardCount[currentPlayer] == P.discardCount[currentPlayer])) {failed = TRUE;}
            if(randomAssert("\tCurrent player's discard array incorrectly modified.", memcmp(&(G.discard[currentPlayer]), &(P.discard[currentPlayer]), sizeof(int) * MAX_DECK) == 0)) {failed = TRUE;}
            if(randomAssert("\tCurrent player's hand array incorrectly modified.", memcmp(&(G.hand[currentPlayer]), &(P.hand[currentPlayer]), sizeof(int) * MAX_HAND) == 0)) {failed = TRUE;}
            if(randomAssert("\tG.supplyCount[] array incorrectly modified.", memcmp(&(G.supplyCount), &(P.supplyCount), sizeof(int) * (treasure_map + 1)) == 0)) {failed = TRUE;}
        }
        ...
        if((WANTS_GOLD == TRUE && HAS_ESTATE == FALSE && ESTATE_IN_SUPPLY == TRUE) || (WANTS_GOLD == FALSE && ESTATE_IN_SUPPLY == TRUE)) {
            ...
            if(randomAssert("\tEstate Supply incorrectly modified.",G.supplyCount[estate] == (P.supplyCount[estate] - 1) )) {failed = TRUE;}
            if(randomAssert("\tTop of current player's discard is not Estate.", G.discard[currentPlayer][P.discardCount[currentPlayer]] == estate)) {failed = TRUE;}
            if(randomAssert("\tG.discardCount[currentPlayer] incorrectly updated.", G.discardCount[currentPlayer] == (P.discardCount[currentPlayer] + 1) )) {failed = TRUE;}
            if(randomAssert("\tG.coins incorrectly modified.", G.coins == P.coins) == FALSE) {failed = TRUE;}
        }

        //Ensure that game state has not been corrupted:
        if(randomAssert("\tgameState.numPlayers corrupted", P.numPlayers == G.numPlayers))                      {failed = TRUE;}
        if(randomAssert("\tgameState.outpostPlayed corrupted", P.outpostPlayed == G.outpostPlayed))             {failed = TRUE;}
        if(randomAssert("\tgameState.outpostTurn corrupted", P.outpostTurn == G.outpostTurn))                   {failed = TRUE;}
        if(randomAssert("\tgameState.whoseTurn corrupted", P.whoseTurn == G.whoseTurn))                         {failed = TRUE;}
        if(randomAssert("\tgameState.phase corrupted", P.phase == G.phase))                                     {failed = TRUE;}
        if(randomAssert("\tgameState.numBuys corrupted", (P.numBuys + 1) == G.numBuys))                         {failed = TRUE;}
        if(randomAssert("\tgameState.playedCardCount corrupted", P.playedCardCount == G.playedCardCount))       {failed = TRUE;}
        ...
        for (int k = 0; k < G.numPlayers; k++) {
            if (k != currentPlayer) {
                ...
                if(randomAssert(message, memcmp(&(G.hand[k]), &(P.hand[k]), sizeof(int) * MAX_HAND) == 0)) {failed = TRUE;};
                sprintf(message, "\tPlayer %d's handCount corrupted (G.handCount[%d])", k, k);
                if(randomAssert(message, P.handCount[k] == G.handCount[k])) {failed = TRUE;};
            }
        }
        ...
        for (int k = 0; k < G.numPlayers; k++) {
            if (k != currentPlayer) {
                ...
                if(randomAssert(message, memcmp(&(G.deck[k]), &(P.deck[k]), sizeof(int) * MAX_HAND) == 0)) {failed = TRUE;};
                sprintf(message, "\tPlayer %d's deckCount corrupted (G.deckCount[%d])", k, k);
                if(randomAssert(message, P.deckCount[k] == G.deckCount[k])) {failed = TRUE;};
            }
        }
        ...
        for (int k = 0; k < G.numPlayers; k++) {
            if (k != currentPlayer) {
                ...
                if(randomAssert(message, memcmp(&(G.discard[k]), &(P.discard[k]), sizeof(int) * MAX_HAND) == 0)) {failed = TRUE;};
                sprintf(message, "\tPlayer %d's discardCount corrupted (G.discardCount[%d])", k, k);
                if(randomAssert(message, P.discardCount[k] == G.discardCount[k])) {failed = TRUE;};
            }
        }
