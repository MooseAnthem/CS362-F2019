...
int main(int argc, char* argv[]) {
    ...
    printf("********************************************************************************\n********************************************************************************\n\t\t\t\t\tBEGINNING RANDOM TEST OF MINION\n********************************************************************************\n********************************************************************************\n");
    ...
    for (int i = 0; i < iterations; i++) {
        ...
        //save game state:
        P = G;
        doMinion(currentPlayer, gainCoin, discardAndRedraw, &G, minionPos);
        ...
        if(randomAssert("\tgameState.numBuys corrupted", P.numBuys == G.numBuys))                                                   {failed = TRUE;}
        if(randomAssert("\tgameState.playedCardCount corrupted", P.playedCardCount == G.playedCardCount))                           {failed = TRUE;}
        if(randomAssert("\tgameState.numActions corrupted/incremented incorrectly", (P.numActions + 1) == G.numActions))            {failed = TRUE;}
        ...
        if( (gainCoin == TRUE && discardAndRedraw == FALSE) || (gainCoin == TRUE && discardAndRedraw == TRUE) ) {
            if(randomAssert("\tgameState.numActions corrupted/incremented incorrectly", (P.numActions + 1) == G.numActions))        {failed = TRUE;}
            if(randomAssert("\tgameState.coins corrupted/incremented incorrectly", G.coins == (P.coins + 2)))                       {failed = TRUE;}
            if(randomAssert("\tplayed Minion card was not removed from hand", G.hand[currentPlayer][minionPos] == minion))          {failed = TRUE;}
            if(randomAssert("\tcurrent player's discardCount corrupted/not changed correctly", G.discardCount[currentPlayer] == (G.discardCount[currentPlayer] + 1))) {failed = TRUE;}
            if(randomAssert("\tcurrentPlayer's top discard is not a Minion", G.discard[currentPlayer][G.discardCount[currentPlayer]]))  {failed = TRUE;}
            ...
            for (int k = 0; k < G.numPlayers; k++) {
                if(k != currentPlayer){
                    ...
                    if(randomAssert(message, memcmp(&(G.hand[k]), &(G.hand[k]), sizeof(int)* MAX_HAND) == 0))                       {failed = TRUE;}
                    sprintf(message, "\tPlayer %d's deck array corrupted/erroneously modified.", k);
                    if(randomAssert(message, memcmp(&(G.deck[k]), &(G.deck[k]), sizeof(int)* MAX_DECK) == 0))                       {failed = TRUE;}
                    sprintf(message, "\tPlayer %d's discard array corrupted/erroneously modified.", k);
                    if(randomAssert(message, memcmp(&(G.discard[k]), &(G.discard[k]), sizeof(int)* MAX_DECK) == 0))                 {failed = TRUE;}
                }
            }
            ...
            for (int k = 0; k < G.numPlayers; k++) {
                if (k != currentPlayer) {
                    if (P.handCount[k] > 4) {
                        ...
                        if(randomAssert(message, (G.handCount[k] == 4)))                                                            {failed = TRUE;}
                    }
                }
            }
            if(randomAssert("\tCurrent player's hand not redrawn.", G.handCount[currentPlayer] == 4))                               {failed = TRUE;}
        ...
        //Case where no valid option is given to the function. No changes should be made to the game state:
            ...
            if(randomAssert("\tgameState.numActions corrupted/incremented incorrectly", P.numActions == G.numActions))              {failed = TRUE;}    //When no valid option is given *no* changes should be made
            if(randomAssert("\tgameState.coins corrupted", G.coins == P.coins))                                                     {failed = TRUE;}
